...
export default defineComponent({
  ...
  setup() {
    ...
    return () => <div class=" h-full flex flex-col items-center justify-center"  >
      <canvas ref={canvasRef} width={width} height={height} onClick={()=>{
        ...
        if(ctxRef.value) draw(ctxRef.value)
      }}  />
      <div class=" flex items-center justify-center gap-5" >
        <div>a:{coefficient.a}</div>
        <div>b:{coefficient.b}</div>
        <div>c:{coefficient.c}</div>
        <div>d:{coefficient.d}</div>
      </div>
    </div>
  },
})
